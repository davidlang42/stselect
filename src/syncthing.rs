...
impl fmt::Display for IgnoreFile {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        writeln!(f, "{} at {:?}", FIRST_LINE_PREFIX, chrono::offset::Local::now())?;
...
impl IgnoreFile {
    pub fn open(root_folder_path: &str) -> Result<Self, Box<dyn Error>> {
        ...
        // check stfolder
        if match fs::metadata(path.with_file_name(STFOLDER)) {
            Ok(stfolder) => !stfolder.is_file(),
            Err(_) => false
        } {
            return Err(Box::new(SyncthingError::MissingStFolder(root_folder_path.to_string())));
        }
        // load existing ignores
        let filename = path.with_file_name(STIGNORE);
        let mut ignore: IgnoreFile = fs::read_to_string(filename.as_path())?.parse()?;
        ignore.filename = Some(filename);
        // compare with actual folders
        let actual = ls(&path, true, false)?;
        let mut i = 0;
    ...
    pub fn save(&self) -> Result<(), Box<dyn Error>> {
        if let Some(filename) = &self.filename {
            if filename.exists() {
                let mut move_to = filename.with_file_name(STRECYCLE);
